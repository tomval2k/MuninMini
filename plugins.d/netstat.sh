...
graph_info State of all TCP & UDP connections reported by netstat -ntau. State definitions taken from 'man netstat'.
...
# in awk script:
#  - at beginning, declare variables
#  - in middle, if field $6 matches regex, increment counter
#  - at end, print out labelled value of counters

netstat -ntau 2> /dev/null | awk 'BEGIN {
ESTABLISHED=0; \
TIME_WAIT=0; \
LISTEN=0; \
SYN_SENT=0; \
